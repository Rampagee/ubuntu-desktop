...
if [ $? -eq 0 ]; then
	  echo "U50 card detected."
	    sudo cp /scratch/alveo_xclbin-1.3.0/U50/6E300M/* /usr/lib
	      detected=1
fi
...
if [ $? -eq 0 ]; then
	  echo "U50LV card detected."
	    sudo cp /scratch/alveo_xclbin-1.3.0/U50lv/10E275M/* /usr/lib
	      detected=1
fi
...
if [ $? -eq 0 ]; then
	  echo "U280 card detected."
	    sudo cp /scratch/alveo_xclbin-1.3.0/U280/14E300M/* /usr/lib
	      detected=1
fi
